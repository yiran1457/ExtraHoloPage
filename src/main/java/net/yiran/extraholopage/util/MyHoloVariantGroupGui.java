...
public class MyHoloVariantGroupGui extends GuiElement {
    ...
    public MyHoloVariantGroupGui(int x, int y, String category, List<OutcomePreview> outcomes, int offset, IStatSorter sorter, Player player,
        ...
        int line = Config.HOLO_MODULE_LINE.get();
        for (int i = 0; i < outcomes.size(); i++) {
            OutcomePreview outcome = outcomes.get(i);
            if (SchematicType.minor.equals(outcome.type)) {
                HoloVariantItemGui variant = new HoloVariantItemGui((i / line) * 15, (i % line) * 15, outcome, sorter.getValue(player, outcome.itemStack),
